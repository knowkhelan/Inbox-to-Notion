...
def generate_texts(raw_input):
    ...
    """
    print(f"🧠 AI Processing: {raw_input[:50]}...") # Print first 50 chars
...
def check_email_and_sync():
    ...
    while True:
        try:
            # print("📧 Checking Email Folder...") # Uncomment to debug
                ...
                for e_id in email_ids:
                    ...
                    for response_part in msg_data:
                        if isinstance(response_part, tuple):
                            ...
                            # --- 🔍 TEST BLOCK START ---
                            print("\n" + "="*50)
                            print(f"✅ SUCCESS: Read Email")
                            print(f"📌 Subject: {clean_subject}")
                            print(f"📝 Body Snippet: {body[:200]}...") # Prints first 200 characters
                            print("="*50 + "\n")
                            # --- 🔍 TEST BLOCK END ---
                            ...
                            # Push to Notion (No specific source link for Email, or use a generic one)
                            push_to_notion(
                                ai_result.get("name", clean_subject), 
                                ai_result.get("description", "Imported from Email"), 
                                ai_result.get("priority", "Medium"),
                                source_link="https://mail.google.com/mail/u/2/#label/NotesTracker" # Generic placeholder
                            )
